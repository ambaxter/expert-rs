...
use crate::shared::compiler::builder::BaseBuilder;
use crate::shared::compiler::builder::ConsequenceBuilder;
use crate::shared::compiler::builder::KnowledgeBase;
use crate::shared::compiler::builder::RuleBuilder;
use crate::shared::compiler::id_generator::{
    ConditionGroupId, ConditionId, IdGenerator, RuleId, StatementGroupId, StatementId,
};
use crate::shared::compiler::prelude::Stage1Compile;
use crate::shared::compiler::prelude::Stage1Node;
use crate::shared::compiler::prelude::{DrainWhere, ProvidesNode};
use crate::shared::fact::{Fact, FactFieldType, GetFieldType, Getter};
use crate::shared::nodes::alpha::HashEqField;
use crate::shared::nodes::alpha::{AlphaNode, IsHashEq};
use crate::shared::nodes::beta::{BetaNode, CollectRequired};
...
impl<T: Fact> ArrayNetworkBuilder<T> {
    fn new() -> ArrayNetworkBuilder<T> {
        ArrayNetworkBuilder {
            alpha_graph: Default::default(),
            beta_graph_map: Default::default(),
        }
    }
}
...
impl ArrayBaseBuilder {
    fn insert_alpha<T: Fact>(&mut self, statement_id: StatementId, nodes: Vec<AlphaNode<T>>) {
        ...
        let (alpha_graph, id_generator) = (
            self.network_builders
                .entry::<ArrayNetworkBuilder<T>>()
    ...
    fn insert_beta<T: Fact>(
        &mut self,
        agenda_group: SymbolId,
        rule_id: RuleId,
        statement_id: StatementId,
        beta_node: &Stage1Node<T>,
    ) -> HashMap<ConditionGroupId, ConditionGroupType> {
        ...
        if !beta_node.is_empty() {
            ...
            let statement_root = match beta_node {
                Any(ref beta_nodes) => Self::insert_beta_group(
                    beta_graph,
                    id_generator,
                    rule_id,
                    statement_id,
                    ConditionGroupType::Any,
                    beta_nodes,
                    &mut condition_groups,
                ),
                NotAny(ref beta_nodes) => Self::insert_beta_group(
                    beta_graph,
                    id_generator,
                    rule_id,
                    statement_id,
                    ConditionGroupType::NotAny,
                    beta_nodes,
                    &mut condition_groups,
                ),
                All(ref beta_nodes) => Self::insert_beta_group(
                    beta_graph,
                    id_generator,
                    rule_id,
                    statement_id,
                    ConditionGroupType::All,
                    beta_nodes,
                    &mut condition_groups,
                ),
                NotAll(ref beta_nodes) => Self::insert_beta_group(
                    beta_graph,
                    id_generator,
                    rule_id,
                    statement_id,
                    ConditionGroupType::NotAll,
                    beta_nodes,
                    &mut condition_groups,
                ),
            ...
            beta_graph
                .statement_root
                .insert(statement_id, statement_root);
        }
        condition_groups
    }

    fn insert_beta_group<T: Fact>(
        beta_graph: &mut BetaGraph<T>,
        ...
        use self::Stage1Node::*;
        let mut children: Vec<ConditionGroupChild> = beta_nodes
            .iter()
            .map(|beta_node| {
                Self::insert_beta_child(
                    beta_graph,
                    id_generator,
                    rule_id,
                    statement_id,
        ...
        let parent_group_id = {
            if !beta_graph.parent_child_rel.contains_right(&children) {
                ...
                beta_graph
                    .parent_child_rel
                    .insert(new_group_id, children.clone());
                new_group_id
            } else {
        ...
        match beta_node {
            ...
            Test(ref node) => {
                if !beta_graph.test_nodes.contains_right(node) {
                    ...
                    ConditionGroupChild::Condition(
                        *beta_graph.test_nodes.get_by_right(node).unwrap(),
                    )
                }
            }
        }
...
impl ArrayRuleBuilder {
    fn add_new_statement<T: Fact, S: AsRef<str>, N: Stage1Compile<T>>(
        &mut self,
        provides: &[ProvidesNode<S, S>],
        nodes: &[N],
    ) -> Result<(StatementId, Box<StatementDetails>), CompileError> {
        ...
        if provided.iter().filter(|p| p.is_variable()).count() > 1 {
            let rule = self
                .base_builder
                .cache
                .resolve(self.rule_data.name)
                .unwrap();
            return Err(CompileError::MultipleVariables {
                rule: rule.to_owned(),
                statement_id,
            });
        }
        ...
        for node in provided {
            match node {
                ProvidesNode::Var(s) => {
                    statement_provides.var = Some(s);
                }
                ProvidesNode::Field(s, g) => {
                    statement_provides.fields.insert(s, g);
                }
            }
        }

        let mut beta_nodes = Stage1Node::All(Stage1Compile::stage1_compile_slice(
            nodes,
            &mut self.base_builder.cache,
        )?)
        .clean();
        ...
        let condition_groups = self.base_builder.insert_beta(
            self.rule_data.agenda_group,
            rule_id,
            statement_id,
            &beta_nodes,
        );
...
impl RuleBuilder for ArrayRuleBuilder {
    ...
    fn provides_when<T: Fact, S: AsRef<str>, N: Stage1Compile<T>>(
        mut self,
        provides: &[ProvidesNode<S, S>],
        nodes: &[N],
        ...
        self.rule_data
            .statement_groups
            .get_mut(&statement_group)
            .unwrap()
            .push(StatementGroupEntry::Statement(statement_id));

        self.rule_data
            .statement_data
            .insert(statement_id, statement_details);
        Ok(self)
    }
    ...
    fn when_exists<T: 'static + Fact, N: Stage1Compile<T>>(
        ...
        let (statement_id, statement_details) =
            self.add_new_statement::<T, &'static str, N>(&[], nodes)?;
        let statement_group = self.rule_data.current_group;
        self.rule_data
            .statement_groups
            .get_mut(&statement_group)
    ...
    fn provides_when_for_all<T: Fact, S: AsRef<str>, N: Stage1Compile<T>>(
        mut self,
        provides: &[ProvidesNode<S, S>],
        nodes: &[N],
    ) -> Result<Self, CompileError> {
    ...
    fn end_group(mut self) -> Result<Self, CompileError> {
        ...
        if current_group_id != parent_id {
            ...
            if current_group.can_single_entry_optimize() {
                ...
                self.rule_data
                    .statement_groups
                    .insert(current_group_id, current_group);
            }
